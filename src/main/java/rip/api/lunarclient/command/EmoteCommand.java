package rip.api.lunarclient.command;

import rip.api.lunarclient.LunarClientAPI;
import rip.api.lunarclient.util.type.Emote;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 1) {
            if (isInteger(args[0])) {
                ...
                if (emoteId == -1) {
                    try {
                        LunarClientAPI.getInstance().performEmote(player, -1, true);
                        return true;
                    } catch (Exception e) {
                ...
                try {
                    LunarClientAPI.getInstance().performEmote(player, emote.getEmoteId(), true);
                } catch (Exception e) {
